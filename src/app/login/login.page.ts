...
export class LoginPage{
  ...
  constructor(public fb: FormBuilder,private router: Router, public alertController: AlertController) {
    this.formLogin = this.fb.group({
      ...
      'pass': new FormControl("", [Validators.required, Validators.minLength(8), Validators.maxLength(15), Validators.pattern(/^(?=.*[a-zA-Z])(?=.*\d)[a-zA-Z\d]{8,}$/)])
  ...
  async ingresar() {
    if (this.formLogin.valid) {
       ...
       const user = users.find((u: any) => u.email === i.email);
   
       if (user && user.pass === i.pass) {
